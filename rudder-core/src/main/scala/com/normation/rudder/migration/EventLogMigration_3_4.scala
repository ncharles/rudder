 ...
 */
class ControlEventLogsMigration_3_4(
    migrationEventLogRepository   : MigrationEventLogRepository
  , eventLogsMigration_3_4        : EventLogsMigration_3_4
  , changeRequestsMigration_3_4   : ChangeRequestsMigration_3_4
  , controlEventLogsMigration_2_3 : ControlEventLogsMigration_2_3
) {
  ...
  def migrate() : Box[MigrationStatus] = {
    ...
    migrationEventLogRepository.getLastDetectionLine match {
      ...
      case Some(status@SerializedMigrationEventLog(
        ...
        logger.info("Start migration of ChangeRequest from format '3' to '4'")

        val changeRequestResult = changeRequestsMigration_3_4.processChangeRequests() match {
          case Full(i) =>
            logger.info("Migration from ChangeRequest fileFormat from '3' to '4' done, %s EventLogs migrated".format(i))
            migrationEventLogRepository.setMigrationFileFormat(status.id, 4, new Timestamp(Calendar.getInstance.getTime.getTime))
        ...
        logger.info("Start migration of EventLog from format '3' to '4'")

        val eventLogResult = eventLogsMigration_3_4.processEventLogs() match {
          case Full(i) =>
        ...
        (changeRequestResult, eventLogResult) match {
          case (Full(MigrationSuccess(i)), Full(MigrationSuccess(j))) => Full(MigrationSuccess(i+j))
          case (errorcr, errorel) => errorcr
        }
      ...
      case Some(status@SerializedMigrationEventLog(
        ...
        controlEventLogsMigration_2_3.migrate() match{
      ...
      case Some(x@SerializedMigrationEventLog(
        ...
        , Some(migrationFileFormat)
        , _
      )) if(migrationFileFormat < 3) =>
        //create a new status line with detected format = migrationFileFormat,
        //and a description to say why we recurse
        controlEventLogsMigration_2_3.migrate()
        this.migrate()
      ...
      case Some(x) =>
        logger.debug("Migration of EventLog from format '3' to '4': nothing to do")
...
class EventLogsMigration_3_4(
  ...
  , eventLogMigration          : EventLogMigration_3_4
  ...
  def findAllEventLogs : Box[Seq[MigrationEventLog]] = {
    ...
    try {
           (xml \\ "@fileFormat" exists { _.text.toInt == 3 })
         } catch {
 ...
 */
class EventLogMigration_3_4(xmlMigration:XmlMigration_3_4) {
  def logger = MigrationLogger(3)
 ...
 */
class XmlMigration_3_4 {

  def changeRequest(xml:Elem) : Box[Elem] = {
    for {
      labelOK      <- TestLabel(xml, "changeRequest")
      fileFormatOK <- TestFileFormat(xml, Constants.XML_FILE_FORMAT_3.toString())
      migrated     <-
                      TestIsElem(
                        (
                        "changeRequest [fileFormat]" #> Constants.XML_FILE_FORMAT_4  &
  ...
  def other(xml:Elem) : Box[Elem] = {
    for {
      ...
      migrated     <- TestIsElem((
                        ...
                        "fileFormat=3 [fileFormat]" #> Constants.XML_FILE_FORMAT_4
                      ).apply(xml))
    } yield {
      migrated
    }
