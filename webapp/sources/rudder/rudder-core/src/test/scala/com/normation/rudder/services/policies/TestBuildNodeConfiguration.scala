...
class TestBuildNodeConfiguration extends Specification {
  ...
  val parallelism               = Parallelism(maxParallelism, Scheduler.io(executionModel = ExecutionModel.AlwaysAsyncExecution), TaskSemaphore(maxParallelism = maxParallelism))
  ...
  "build node configuration" in {
    ...
    for(i <- 0 until 10) {
      ...
      val nodeContexts = buildContext.getNodeContexts(allNodes.keySet, allNodes, groupLib, Map(), Seq(), data.globalAgentRun, data.globalComplianceMode, globalPolicyMode).getOrElse(throw new RuntimeException("oups"))
      val t3 = System.currentTimeMillis()
      val config = BuildNodeConfiguration.buildNodeConfigurations(activeNodeIds, ruleVals, nodeContexts, allNodeModes, scriptEngineEnabled, globalPolicyMode, parallelism, jsTimeout, generationContinueOnError)
