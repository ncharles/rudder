...
class TestBuildNodeConfiguration extends Specification {
  ...
  "build node configuration" in {
    ...
    for(i <- 0 until 10) {
      ...
      val config = BuildNodeConfiguration.buildNodeConfigurations(activeNodeIds, ruleVals, nodeContexts, allNodeModes, scriptEngineEnabled, globalPolicyMode, maxParallelism, jsTimeout, generationContinueOnError)
